...
6. **State Commitment** (`sbo-core/src/state/db.rs::object_to_segments`). `creator` is a literal trie segment; revisit against the State Commitment spec if the owner-model change touches segment derivation.
