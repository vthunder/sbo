...
- **Certs:** use the live **browserid.sandmill.org** instance for real certs; **tests use mocks** (mock evidence + a test pinned KSK).
- **DNSSEC deployment — CONFIRMED REAL (2026-06-23):** `sandmill.org` is DNSSEC-signed (DNSKEY alg 13 ECDSA-P256; DS `59066 13 1 5F95E0F94DACB020420E09286B3CB0118D928604` at parent → chains to root; Cloudflare 1.1.1.1 sets the AD flag → validates). `_browserid.sandmill.org` TXT exists: `v=browserid1; public-key-algorithm=Ed25519; public-key=sjL09EuHfTlyzzvVe53lCHV8LHeta4q6KMnjxRAO7ZI`, with RRSIG (alg 13, key tag 57592, window 2026-06-11→07-02). So **real DNSSEC evidence is capturable** for the verifier/capture path; pin the well-known IANA **root KSK**. WRINKLE: HTTPS to `browserid.sandmill.org` failed TLS hostname verification (cert subject mismatch) — the cert-issuance HTTP endpoint hostname/cert needs sorting (or the endpoint is elsewhere). Affects live cert acquisition only, not the DNSSEC verifier. Until sorted: mock cert issuance, real DNSSEC evidence.
