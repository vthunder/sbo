...
### Phase 4 — Policy extension  ✅ DONE (2026-06-24)
...
**Implemented:** `AttestedSource{type, by?}` + `Identity::Attested` variant + `Requirements.attested`/`not_attested` (`policy/types.rs`). `evaluate()`/`identity_matches`/`check_requirements` take an `is_attested: &dyn Fn(&AttestedSource)->bool` closure (kept pure; tests pass a stub). The daemon builds it: `attested_subject_matches` enumerates attestations (prefix-scan `/<by>/attestations/` when `by` is given, else a schema scan of all `attestation.v1`), and matches type + in-force-at-inclusion-time + issuer-controller (`by`) + subject-controller == the requester's controller (attributed email else signer key). New `StateDb::list_objects_by_path_prefix`/`list_objects_by_schema`. Deterministic (all on-chain + inclusion-time-pinned). **Spec fix:** the core §Identifier Syntax forbade `:`/`@`, but attestation type IDs need `:` (`role:moderator`) and emails need `@`; since the key delimiter migrated to `0x1F`, both are now allowed in `Id` and the grammar was reconciled. Tests: core role/condition matching via stub; daemon end-to-end attestation-gated role (attested moderator allowed, stranger denied).

### Phase 5 — Community — **DONE** (on `main`)
...
- **5.0 — `post`⇒{create,update} expansion** (`policy/evaluate.rs`). Replaced the exact `grant.can.contains(&action)` match with `action_covered_by(granted, requested)`: a `post` grant now authorizes `create` and `update` requests (Policy Spec: `post` = shorthand for create+update); `*` covers all; the create/update split stays asymmetric (a bare `create` grant does **not** confer update). Tests: `post_grant_covers_create_and_update_not_delete`, `create_grant_does_not_imply_update`. Closes the post-Phase-4 carry-forward gap.
- **5.1 — `community.v1` schema** (`sbo-core/src/schema/community.rs`, mirrors `attestation.rs`; wired into `schema/mod.rs::validate_schema`). `Community{name,issuer,policy, description?, members?, spaces?, open?, created_at?}`; `validate_community` checks required-string presence/non-emptiness + serde type-checks only — pointers (`issuer`/`policy`) are **not** dereferenced (resolve at read time); storage location is convention. `members_prefix()`/`spaces_prefix()` apply the `/members/`,`/spaces/` defaults; `DEFAULT_MEMBERS_PREFIX`/`DEFAULT_SPACES_PREFIX` exported. Unit + `validate_schema`-level tests.
- **5.2 — end-to-end open community integration test** (`sbo-daemon/tests/l2_authorization.rs::open_community_membership_post_and_ban_end_to_end`). Composes a `community.v1` descriptor (validated at genesis), an open-community root policy (`member` role = in-force `membership` attestation, `post` grant on `/spaces/**`, `not_attested` ban-by-issuer restriction): join → post (exercises post⇒create), stranger → denied, ban → denied. New `put_attestation` helper.
...
**6.5 — tip vs confirmed: DEFERRED by product decision** (2026-06-24). Not built. Rationale fully captured in `docs/plans/2026-06-24-demo-ux-spec.md` §6: in SBO's based/no-sequencer model tip is the author's own optimistic echo, so it belongs **client-side** (a pure `sbo-core` outbox+overlay reusing `hlc::lww_wins`), **never a daemon mempool** (confirmed state must stay globally deterministic). The Mingo demo doesn't need it (a couple-second post latency is fine for Reddit-style use); tip is deferred to a future low-latency feature (chat/live). The protocol primitives it would build on (`Hlc`, `lww_wins`) already exist.

### Phase 7 — Reference client = the **Mingo** demo (see `docs/plans/2026-06-24-demo-ux-spec.md`)
The demo is locked: **Mingo**, a multi-community hub in **one aggregated repo**, with Mingo as the **meta-community + T1 identity provider** (one pseudonymous `<name>@mingo` per user). Product spine = the **Reputation Passport** (portable earned attestations, a local in-repo read); pitch = "Reddit, except you keep what you build" (no blockchain/verification in the pitch). **Phase 6 leaves no protocol work for the demo** — remaining work is client + provider:
1. **Mingo provider + aggregated genesis** (browserid provider for `@mingo`; hub root policy + starter communities `cooks`/`woodworking`/`homelab` with `community.v1` + policies).
...
- ~~**`post` action does not expand to `create`/`update` in policy `can`.**~~ **RESOLVED in Phase 5.0** — `policy/evaluate.rs::action_covered_by` expands `post`⇒{create,update}.
