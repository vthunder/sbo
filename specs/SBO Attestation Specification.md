...
> **Decision: issuer namespace, not subject namespace.** An attestation must be controlled by the party who made the claim. If attestations lived in the subject's namespace, a subject could delete or suppress unfavorable claims about themselves — a ban or a revoked credential would be worthless. Storing under the issuer's namespace means the issuer's authorization governs create, update, and revoke; the subject can neither forge nor remove claims about themselves. The cost — that "all attestations about Alice" is not a single on-chain subtree — is borne by indexers, which build the reverse index (see [Indexing](#indexing-and-off-chain-scoring)). Placing the issuer in the **path** is also what makes attestations compatible with global `(path, id)` uniqueness: two different issuers asserting the same `(subject, type)` live at different paths and so never contend for a slot — coexistence comes from the path, not from sharing a slot with distinct creators.
